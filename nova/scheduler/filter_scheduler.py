...
class FilterScheduler(driver.Scheduler):
    ...
    def _get_sorted_hosts(self, spec_obj, host_states, index):
        ...
        """
        # APSR: choose from D hosts list
	filtered_hosts = self.host_manager.get_filtered_hosts(host_states,
            spec_obj, index)
	
	# APSR  - update flavor counters
	p = re.compile("flavor=Flavor(.*),force_hosts=")
	line = ("%(hosts)s",{'hosts': spec_obj})
	result = p.search(str(line))
	flavor_id = result.group(1)[1:-1]
	flavor_id = int(flavor_id)
	
	#update counters
	LOG.info("APSR: update dict with: %d %d",flavor_id,len(filtered_hosts))	
	apsr_controller.update_dict_file(flavor_id,len(filtered_hosts)) 
