...
class apsr_scheduler(driver.Scheduler):
    """Scheduler that can be used for filtering and weighing."""
    def __init__(self, *args, **kwargs):
        super(apsr_scheduler, self).__init__(*args, **kwargs)
	...
	LOG.info("OR & LIRON 1")
        time.sleep(10)
	LOG.info("APSR: schdualer start prossesing request")
	...
	#choss from d host 
	#d_host = 2
	#if len(host_states) > d_host:
	#	hosr_states = random.sample(host_states ,d _host)
        ...
        LOG.info("APSR: Weighed %(hosts)s", {'hosts': weighed_hosts})
