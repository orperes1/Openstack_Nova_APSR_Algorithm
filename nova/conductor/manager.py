...
from nova.scheduler import apsr_controller
...
class ComputeTaskManager(base.Base):
	...
	LOG.info("APSR: liron and or test 5555_3")
	open_new_s = apsr_controller.open_s()
	LOG.info("APSR: s will open? %d",open_new_s)
	while (open_new_s == 0):
	    time.sleep(1)
	    open_new_s = apsr_controller.open_s()
	    LOG.info("sch will open - %d",open_new_s)

	x = Thread(target=self.schedule_and_build_instances_th ,
					args=(context, build_requests,
                                        request_specs, image,
                                        admin_password, injected_files,
                                        requested_networks, block_device_mapping,))
	x.start()
	
    def schedule_and_build_instances_th(self, context, build_requests,
                                     	request_specs, image,
                                     	admin_password, injected_files,
                                     	requested_networks, block_device_mapping,
                                    	 tags=None):
	...
	LOG.info("APSR: liron and or test 5555_1")
        # First create the action record.
        try:
