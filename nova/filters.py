...
Filter support
"""
from scheduler import apsr_controller
import random
...
class BaseFilterHandler(loadables.BaseLoader):
    ...
    def get_filtered_objects(self, filters, objs, spec_obj, index=0):
        list_objs = list(objs)
	d_val = apsr_controller.get_d()
        
	if len(list_objs) > d_val:
	    list_objs = random.sample(list_objs , d_val)
	LOG.info("Starting with %d host(s)", len(list_objs))
